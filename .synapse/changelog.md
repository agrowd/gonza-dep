...
- Actualización dinámica de precios según listado de zonas y mejoras visuales en Detalle del Turno (`src/app/admin/agenda/page.js`):
  1. Recálculo automático de precios actuales: al abrir la ficha del turno, el sistema consulta el catálogo actual de zonas (`Zona`) y recalcula el Valor Original y el Valor Total según los precios vigentes, respetando fielmente el descuento del cliente (ya sea en porcentaje o en monto fijo en pesos) y conservando la seña pagada.
  2. Zonas a depilar en negrita destacada (`font-weight: 800; font-size: 1.05rem; color: var(--text-primary)`): resaltadas prominentemente para que el operador identifique al instante las áreas a tratar.
  3. Contraste óptico en Valor Original: se sustituyó el color blanco por texto oscuro de alto contraste (`color: var(--text-primary); font-size: 1.2rem; font-weight: 800`), eliminando cualquier problema de legibilidad sobre fondos grises.
