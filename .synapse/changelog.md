...
- Corrección del filtrado de turnos en la grilla tras abrir Ficha Cliente (`src/app/admin/agenda/page.js`): se implementó el helper `getAppDateStr` para interpretar y convertir correctamente las cadenas ISO de fecha recibidas desde Prisma (`app.fecha`) a la fecha local de Argentina (UTC-3). Esto soluciona que al volver de la Ficha del Cliente la grilla apareciera en blanco por desajuste entre el string UTC de la base de datos y la fecha seleccionada en pantalla.
