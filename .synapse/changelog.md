...
- Preservación del precio total base al presionar **Programar Siguiente Turno** (`handleScheduleNextTurn` en `src/app/admin/agenda/page.js`): se inicializa `manualTotalOverride` con la suma del precio cobrado más la bonificación ($30.000), impidiendo que al agendar la sesión siguiente (4 u 8 semanas después) el 20% se aplique acumulativamente sobre el monto con descuento previo ($24.000 -> $19.200).
- Solución al problema de carga inicial de turnos en la agenda (`src/app/admin/agenda/page.js`): se reestructuró la función `fetchAppointments` utilizando `useCallback` con cálculo directo de los rangos de fecha a partir de `selectedDate` y `currentWeekStart`, eliminando la dependencia condicional `weekDates.length === 0` que abortaba la consulta de turnos en la primera renderización y obligaba al usuario a cambiar de vista para ver sus turnos.
- Corrección del corte de medianoche en el módulo de **Estadísticas** (`/api/admin/estadisticas`): las ganancias del día (`gananciaDia`), la semana (`gananciaSemana`) y el mes (`gananciaMes`) ahora calculan los rangos de fecha utilizando strictly la hora local de Argentina (UTC-3). Esto evita que a las 21:00 hs (00:00 UTC) las ganancias del día vuelvan a $0 por cambio de fecha en UTC.
