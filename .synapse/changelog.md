...
- Conversión del logo a versión blanca de alto contraste en el panel administrativo (`src/app/admin/SidebarNav.js`): se aplicó el filtro `brightness(0) invert(1)` exclusivamente a la barra lateral del admin, transformando el texto y la gráfica del logo a color blanco brillante para lograr una lectura 100% legible y un alto contraste perfecto sobre el fondo granate.
- Eliminación del juego/desplazamiento horizontal en dispositivos móviles (`overflow-x: hidden` y `max-width: 100vw`): se configuraron las reglas globales e internas de contenedor en `src/app/admin/layout.module.css`, `notificaciones.module.css`, `estadisticas.module.css`, `configuracion.module.css` y `clientes.module.css`, encapsulando tablas y paneles para impedir desbordamientos o bamboleos laterales al desplazarse en celulares.
- Elevación de la sección de usuario y botón de **Cerrar Sesión** en la barra lateral móvil (`src/app/admin/layout.module.css`): se añadió `padding-bottom: calc(2.5rem + env(safe-area-inset-bottom, 0px))` y `margin-bottom: 1rem` en la regla `@media (max-width: 768px)`, evitando que la barra de navegación táctil o botones de Android y gestos de iOS tapen el botón de salida.
