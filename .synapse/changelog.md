...
- Corrección de alineación del modal en celulares y botón de cierre táctil (`src/app/admin/agenda/agenda.module.css`): se configuró `align-items: flex-start`, `margin: 0 auto`, `overflow-x: hidden` y `word-break: break-word` en `.modalOverlay` y `.modalContent`. Esto resuelve que la ventana flotante se desplace lateralmente o aparezca cortada a la izquierda en pantallas móviles. Asimismo, se convirtió el botón de cierre `×` (`.closeBtn`) en un elemento circular táctil de 42px con alto contraste y respuesta rápida al toque.
