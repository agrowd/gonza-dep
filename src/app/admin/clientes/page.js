...
function ClientesPageContent() {
  ...
  const handleOpenResendEmail = (turno) => {
    ...
    setResendEmailAddress(selectedClient?.email || turno?.cliente?.email || editNotes.email || '');
  ...
  const handleSendWppNotice = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(data.message || `Mensaje de WhatsApp enviado correctamente a ${resendWppPhone}.`, 'success');
        if (selectedClient) {
      ...
      showToast('Error de red al enviar el WhatsApp.', 'error');
    } finally {
                          ...
                          return selectedClient.turnos.map((t, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {Boolean(t.notasGonzalo || (t.notasGonzalo === undefined && selectedClient.notasGonzalo)) && (
                                  <div style={{
                                    marginTop: '0.6rem',
                                    backgroundColor: 'rgba(212, 165, 77, 0.08)',
                                    ...
                                    <div style={{ fontSize: '0.82rem', color: 'var(--text-primary)', whiteSpace: 'pre-wrap', lineHeight: 1.35 }}>
                                      {t.notasGonzalo !== null && t.notasGonzalo !== undefined ? t.notasGonzalo : selectedClient.notasGonzalo}
                                    </div>
                                  </div>
                                )}
                                {t.observaciones && (
                                  <div style={{
                                    marginTop: (t.notasGonzalo || selectedClient.notasGonzalo) ? '0.4rem' : '0.6rem',
        ...
        <div className={agendaStyles.modalOverlay} style={{ zIndex: 10000 }}>
          <div className={`glass-card premium-border ${agendaStyles.modalContent}`} style={{ maxWidth: '520px', width: '100%', boxSizing: 'border-box' }}>
            <div className={agendaStyles.modalHeader}>
              ...
              <button type="button" onClick={() => setResendEmailModalTurno(null)} className={agendaStyles.closeBtn}>&times;</button>
            </div>
            ...
            <form onSubmit={handleSendEmailNotice} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', padding: '0.5rem 0 1rem 0' }}>
              <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.04)', padding: '0.85rem 1rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                  {selectedClient?.nombreCompleto || resendEmailModalTurno.cliente?.nombreCompleto || 'Cliente'}
                </div>
              ...
              <div className={agendaStyles.modalFooter} style={{ marginTop: '0.5rem', display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
                <button
                  type="button"
        ...
        <div className={agendaStyles.modalOverlay} style={{ zIndex: 10000 }}>
          <div className={`glass-card premium-border ${agendaStyles.modalContent}`} style={{ maxWidth: '520px', width: '100%', boxSizing: 'border-box' }}>
            ...
            <form onSubmit={handleSendWppNotice} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem', padding: '0.5rem 0 1rem 0' }}>
              <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.04)', padding: '0.85rem 1rem', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--text-primary)' }}>
                  {selectedClient?.nombreCompleto || resendWppModalTurno.cliente?.nombreCompleto || 'Cliente'}
                </div>
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
